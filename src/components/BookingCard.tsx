  ...
  const makeBooking = async () => {
    try{
      if(pickUpDate.current&&returnDate.current&&session?.user.token){
        ...
        const response = await createCarBooking( CarBookingItem.providerId, CarBookingItem.pickUpDate, CarBookingItem.returnDate, CarBookingItem.token);
  ...
  return (
    <div className={styles.booking}>
      <div className="flex flex-row justify-around my-20 mx-20 px-20 py-20">
        ...
        <CarCardCustom model={model} provider={name} price={price} imageURL={imageURL} carId={cid}/>
